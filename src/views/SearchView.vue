    ...
    <div class="container-border">
      <div class="shadow">
        ...
        <div class="pb-3" v-else-if="results.length !== 0 && query.length >= 3">
          ...
          <div class="flex relative mx-3 hover:bg-gray-200 dark:hover:bg-slate-700 mt-3 ps-1 lg:ps-3" v-for="result in results" :key="result.id">
            ...
            <div class="ms-3 my-auto text-gray-700 dark:text-gray-300 text-xl w-full">
              <div class="flex">
                <router-link :to="'/' + result.username + '/'" class="hover:text-gray-600 dark:hover:text-gray-400">
                  {{result.username}}
                </router-link>
                <button
                    class="btn-follow ms-3 px-5"
                    type="button"
                    :class="{'bg-gray-500 dark:bg-gray-600': result.is_followed, 'bg-blue-500': !result.is_followed}"
              ...
              <div class="flex text-xl text-gray-600 dark:text-gray-400 pb-1 mt-auto">
                <div class="pr-4">
                  <div class="text-base lg:text-lg">
                    <span class="text-gray-700 dark:text-gray-300 font-semibold text-xl varela-round">{{result.posts_count}}</span>
                    posts
                  </div>
                </div>
                <div class="pr-4">
                  <div class="text-base lg:text-lg">
                    <span class="text-gray-700 dark:text-gray-300 font-semibold text-xl varela-round">{{result.followers_count}}</span>
                    followers
                  </div>
                </div>
                <div>
                  <div class="text-base lg:text-lg">
                    <span class="text-gray-700 dark:text-gray-300 font-semibold text-xl varela-round">{{result.followings_count}}</span>
                    followings
                  </div>
                </div>
              </div>
        ...
        <div class="text-gray-600 dark:text-gray-500 pb-10" v-else-if="query.length < 3">
