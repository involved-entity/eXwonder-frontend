...
export const useAccountStore = defineStore('account', {
    actions: {
        ...
        async resetPasswordConfirm(uid: string, token: string, password1: string, password2: string): Promise<IResponse> {
            return await request(Methods.POST, '/api/v1/account/password-reset-confirm/', {
                uid,
                token,
        ...
        async changePassword(oldPassword: string, newPassword1: string, newPassword2: string): Promise<IResponse> {
