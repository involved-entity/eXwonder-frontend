...
export const useAuthenticationStore = defineStore("authentication", {
    ...
    actions: {
        async login(username: string, password: string): Promise<IResponse> {
            return await request(Methods.POST, '/api/v1/account/account/login/', {
                username, password
        ...
        async logout(): Promise<void> {
            await request(Methods.GET, '/api/v1/account/account/logout/', undefined, undefined, axios.HttpStatusCode.NoContent)
            this.token = ''
