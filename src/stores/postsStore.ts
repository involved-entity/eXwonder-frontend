...
export const usePostsStore = defineStore("posts", {
    actions: {
        async createPost(formData: IPostCreate): Promise<IResponse> {
            return await request(Methods.POST, '/api/v1/posts/posts/', formData, {headers: {'Content-Type': 'multipart/form-data'}}, axios.HttpStatusCode.Created)
        },
        ...
        async addPostToSaved(postId: number): Promise<IResponse> {
            return await request(Methods.POST, '/api/v1/posts/saved/', {post_id: postId}, undefined, axios.HttpStatusCode.Created)
        },
        async removePostFromSaved(postId: number): Promise<IResponse> {
            return await request(Methods.DELETE, `/api/v1/posts/saved/${postId}/`, undefined, undefined, axios.HttpStatusCode.NoContent)
        },
        ...
        async deletePost(postId: number): Promise<IResponse> {
            return await request(Methods.DELETE, `/api/v1/posts/posts/${postId}/`, undefined, undefined, axios.HttpStatusCode.NoContent)
        },
        async addPostLike(postId: number): Promise<IResponse> {
            return await request(Methods.POST, `/api/v1/posts/post-likes/`, {post_id: postId}, undefined, axios.HttpStatusCode.Created)
        },
        async deletePostLike(postId: number): Promise<IResponse> {
            return await request(Methods.DELETE, `/api/v1/posts/post-likes/${postId}/`, undefined, undefined, axios.HttpStatusCode.NoContent)
        },
