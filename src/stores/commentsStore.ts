...
export const useCommentsStore = defineStore("comments", {
    actions: {
        ...
        async deleteComment(commentId: number): Promise<IResponse> {
            return await request(Methods.DELETE, `/api/v1/posts/comments/${commentId}/`, undefined, undefined, axios.HttpStatusCode.NoContent)
        },
        async addCommentLike(commentId: number): Promise<IResponse> {
            return await request(Methods.POST, '/api/v1/posts/comment-likes/', {comment_id: commentId}, undefined, axios.HttpStatusCode.Created)
        },
        async deleteCommentLike(commentId: number): Promise<IResponse> {
            return await request(Methods.DELETE, `/api/v1/posts/comment-likes/${commentId}/`, undefined, undefined, axios.HttpStatusCode.NoContent)
        }
    }
})
