<template>
  <div class="fixed inset-0 bg-opacity-90 bg-black flex justify-center items-center z-50">
    <div class="flex h-5/6 text-gray-700 dark:text-gray-300">
      ...
      <div class="rounded-2xl overflow-y-auto overflow-x-hidden flex flex-col mb-auto max-h-full lg:w-[21rem] xl:w-[24rem] bg-gray-200 dark:bg-[#111]" ref="commentsBlock">
        <div class="sticky top-0 left-0 pt-3 pb-1 bg-gray-300 dark:bg-[#070707]">
          <div class="grid grid-cols-2 mx-3">
            ...
            <div class="flex col-span-1 ms-auto">
              <div class="mr-1" v-if="post.author.id === authenticationStore.user.id">
                ...
                <div id="dropdownMenu" class="z-10 hidden shadow-xl shadow-slate-500/30 dark:shadow-none bg-gray-300 dark:bg-[#1b1919] divide-y divide-gray-100 border-red rounded w-36">
          ...
          <div class="m-2 w-full grid grid-cols-8">
            ...
            <div class="ms-3 col-span-6">
              ...
              >
                <router-link :to="'/' + post.author.username + '/'" class="hover:text-gray-600 dark:hover:text-gray-400" active-class="" @click="routeStore.changeActiveLink('user')">
                  {{post.author.username}}
                </router-link>
                <p class="text-gray-600 dark:text-gray-400 text-sm ms-2 montserrat">{{post.time_added.time_added}} ago</p>
              </div>
              <div class="text-gray-600 dark:text-gray-500 text-sm" v-if="post.signature.length && post.signature.length <= 100">{{post.signature}}</div>
              <div
        ...
        <div class="sticky bottom-0 left-0 w-full py-3 grid grid-cols-12 bg-gray-300 dark:bg-[#070707] h-[28%]">
